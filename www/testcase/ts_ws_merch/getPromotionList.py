...
from www.common.webservice import webservice
        ...
        for p in range(1,page+2):
            ...
            for i in range(0,len(MerchListPage.model['promotionListModel'])):
                if MerchListPage.model['promotionListModel'][i]['merchName'].encode('utf-8') == self.Merch1.fullName:
                    time+=1
            if p == page+1:
                self.assertEqual(len(allMerchList.model['promotionListModel']),numMerch%4,"The last page is wrong")
            else:
                self.assertEqual(len(allMerchList.model['promotionListModel']),4,"Every page is wrong")
