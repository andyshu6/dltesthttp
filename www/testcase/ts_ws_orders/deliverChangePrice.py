...
class deliverChangePrice(unittest.TestCase):
    ...
    def test_ReduceChangeOrderPrice_cashondelivery(self):
        ...
        # 货到付款订单减价调用接口
        ChangePriceRst = ws.changeOrderPrice(orderNo=self.UserShop.orderNo,orderDiscountAmount='100', orderChangeAmount=str(
        ...
        # 检验订单减价后是否修改成功
        RstDb1 = select_one('select * from dlorder.dl_order_orderinfo where order_no = ?', self.UserShop.orderNo)
        self.assertEquals(RstDb1.order_amount, RstDb.order_retail_amount - 100)
        update('update dlorder.dl_order_orderinfo set order_amount = ? where order_no = ?',RstDb.order_retail_amount, self.UserShop.orderNo)
    ...
    def test_AddChangeOrderPrice_cashondelivery(self):
        ...
        RstDb = select_one('select * from dlorder.dl_order_orderinfo where order_no = ?', self.UserShop.orderNo)
        # 货到付款订单加价调用接口
        ChangePriceRst = ws.changeOrderPrice(orderNo=self.UserShop.orderNo, orderDiscountAmount='-100',
    ...
    def test_changeOrderPrice_cashondelivery(self):
        ...
        #货到付款订单加价
        ChangePriceRst1 = ws.changeOrderPrice(orderNo=self.UserShop.orderNo, orderDiscountAmount='-200',orderChangeAmount=str(RstDb1.order_retail_amount+200),
        ...
        #检验订单加价
        RstDb2 = select_one('select * from dlorder.dl_order_orderinfo where order_no = ?', self.UserShop.orderNo)
    ...
    def test_RestoreOriginalPrice_cashondelivery(self):
        ...
        #减价后恢复原价
        ChangePriceRst1 = ws.changeOrderPrice(orderNo=self.UserShop.orderNo,orderDiscountAmount='0', orderChangeAmount=str(RstDb1.order_retail_amount),orderStatus=RstDb1.order_status)
        self.assertEquals(ChangePriceRst1.code,200)
        self.assertEquals(ChangePriceRst1.model['success'], '0')
        #检验是否恢复原价
        RstDb2 = select_one('select * from dlorder.dl_order_orderinfo where order_no = ?', self.UserShop.orderNo)
...
def suite():
    ...
    suite.addTest(deliverChangePrice('test_AddChangeOrderPrice_cashondelivery'))
    suite.addTest(deliverChangePrice('test_ReduceChangeOrderPrice_cashondelivery'))
    suite.addTest(deliverChangePrice('test_changeOrderPrice_cashondelivery'))
    suite.addTest(deliverChangePrice('test_AlreadypaidChangeOrderPrice_cashondelivery'))
    suite.addTest(deliverChangePrice('test_AlreadCancelChangeOrderPrice_cashondelivery'))
    suite.addTest(deliverChangePrice('test_RestoreOriginalPrice_cashondelivery'))
    suite.addTest(deliverChangePrice('test_AddChangeOrderPrice_Onlinepayment'))
    suite.addTest(deliverChangePrice('test_ReduceChangeOrderPrice_Onlinepayment'))
    suite.addTest(deliverChangePrice('test_changeOrderPrice_Onlinepayment'))
    suite.addTest(deliverChangePrice('test_AlreadypaidChangeOrderPrice_Onlinepayment'))
    suite.addTest(deliverChangePrice('test_DeliverGoodsChangeOrderPrice_Onlinepayment'))
    suite.addTest(deliverChangePrice('test_ReceivedGoodsChangeOrderPrice_Onlinepayment'))
    suite.addTest(deliverChangePrice('test_AlreadCancelChangeOrderPrice_Onlinepayment'))
    suite.addTest(deliverChangePrice('test_RestoreOriginalPrice_Onlinepayment'))
    return suite
